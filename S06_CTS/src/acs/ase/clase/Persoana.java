...
public class Persoana implements IPersoana{
	...
	public int getVarsta(){
		...
		if(an==0) {
			throw new IllegalArgumentException("Nu se poate determina anul");
		}
		...
		dataNasterii.set(an,luna-1,zi);
		Calendar dataCurenta=Calendar.getInstance();
		long varsta_zile=TimeUnit.MILLISECONDS.toDays(dataCurenta.getTimeInMillis()-dataNasterii.getTimeInMillis());
		
		if(varsta_zile <0) {
			throw new IllegalArgumentException("Persoana nu este inca nascuta");
		}
	...
	public boolean checkCNP(){
		...
		if(CNP==null) {
			throw new IllegalArgumentException("CNP-ul este null");
		}
		if(CNP.length()!=13) {
			throw new IllegalArgumentException("CNP-ul nu are lungimea corecta");
		}
		if(CNP.charAt(0)=='0') {
			throw new ExceptieCNP("Prima cifra din CNP nu poate fi 0!");
		}
		...
		{
			throw new ExceptieCNP("CNP-ul contine caractere incorecte");
		}
